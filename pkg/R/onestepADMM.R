...
#' @param C the matrix that linearly constrains the regression coefficients, e.g. a 1 by p
...
onestepADMM <- function(x, y, family = c("gaussian", "cox", "poisson", "logistic"),
  ...
  if (is.null(u1_0)){
    #u1 = 0
    u1 = matrix(0, nrow = nrow(C), ncol = 1)
  } else{
    u1 = u1_0
  }
  ...
  for(k in 1:max.iter){
    ...
    eps_pri = sqrt(p) * abs.tol + rel.tol *
      max(vector.2.norm(rbind(C, diag(p)) %*% x), vector.2.norm( - z), vector.2.norm(d))
    #eps_pri
    #eps_dual = sqrt(p) * abs.tol + rel.tol * vector.2.norm(rho * (u1 + u2))
    eps_dual = sqrt(nobs) * abs.tol + rel.tol * vector.2.norm(t(rbind(C, diag(p))) %*% rbind(u1, u2))
    #eps_dual
