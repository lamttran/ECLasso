...
KKTtestfct = function(par_guess, family = c("logistic", "cox"), lambda, x, y = NULL, 
  ...
  if(length(constrained.index) > 0){
    if(nrow(C) == 1){
      if(trace == F){
        ...
        eta_lower = round((-gradvec[constrained.index] - lambda)/c(C), 5)
        eta_upper = round((-gradvec[constrained.index] + lambda)/c(C), 5)
  ...
  if(trace == F){
    ...
    return(list(kkt.logical = kkt.logical, failed.indices = failed.indices))
  }
}
