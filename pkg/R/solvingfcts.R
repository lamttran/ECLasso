...
fast_constrained <- function(x, y, censor = NULL, C = NULL, d = NULL,
  ...
  if(family == "logistic"){
    censor = NULL
    fit1 = glmnet(x = x, y = y, family = "binomial", intercept = intercept, standardize = F,
  ...
  #Padding a column for intercept if required
  if (intercept == T) {
    x = cbind(1, x)
    C = cbind(0, C)
  ...
  #The maximum depth can be is the length of the glmnet lambda sequence
  depth = min(depth, length(fit1$lambda))
  ...
  for(i in 1:depth){
    ...
    if(family == "logistic" & intercept == F){
      index_i = par_keep[[unique_index[min(i.glmnet, nlambda)]]] - 1
    } else if(family == "logistic" & intercept == T){
  ...
  if(trace == F){
    ...
    return(list(solution = solution, time = time.iter, depth = depth, 
                subsets = i.glmnet.iter, lambda_seq = lambda_seq[1:depth]))
  }
}
...
naive_fit <- function(x, y, censor = NULL, C = NULL, d = NULL,
  ...
  if(trace == F){
    ...
    return(list(solution = solution, time = time.iter, 
                depth = depth, lambda_seq = lambda_seq[1:depth]))
  }
} 
